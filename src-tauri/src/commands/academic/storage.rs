...
impl PaperStorage {
    ...
    pub async fn add_paper(
...

        {
            ...
            let existing_id = data
                ...
                .iter()
                .find(|(_, p)| {
                    p.paper.metadata.doi == library_paper.paper.metadata.doi
                        && library_paper.paper.metadata.doi.is_some()
        ...
        if let Some(f) = filter {
            ...
            if let Some(ref tags) = f.tags {
                papers.retain(|p| {
                    p.tags
                        .as_ref()
                        .map(|t| tags.iter().any(|tag| t.contains(tag)))
                        .unwrap_or(false)
                });
            }
    ...
    pub fn get_pdf_path(&self, paper_id: &str) -> Result<Option<String>, String> {
        ...
        Ok(data
            .papers
            .get(paper_id)
            .and_then(|p| p.local_pdf_path.clone()))
    }
    ...
    pub async fn import_papers(
        ...
        let papers_to_import: Vec<Paper> = match format {
            ...
            "json" => serde_json::from_str(data).map_err(|e| format!("Invalid JSON: {}", e))?,
        ...
        for paper in papers_to_import {
            match self
                .add_paper(paper.clone(), options.target_collection.clone())
                .await
            {
                Ok(_) => imported += 1,
                Err(e) => {
                    if e.contains("already exists") {
                        match options.merge_strategy.as_str() {
                            ...
                            "replace" => {
                                ...
                                match self
                                    .add_paper(paper, options.target_collection.clone())
                                    .await
                                {
                                    Ok(_) => imported += 1,
                                    Err(e) => {
    ...
    pub async fn export_papers(
        ...
        let papers: Vec<&LibraryPaper> = if let Some(ids) = paper_ids {
            ids.iter().filter_map(|id| data.papers.get(id)).collect()
    ...
    pub async fn get_statistics(&self) -> Result<AcademicStatistics, String> {
        ...
        for paper in data.papers.values() {
            *stats
                .papers_by_status
                .entry(paper.reading_status.clone())
                .or_insert(0) += 1;
            *stats
                .papers_by_provider
                .entry(paper.paper.provider_id.clone())
                .or_insert(0) += 1;
...
fn generate_bibtex(papers: &[&LibraryPaper]) -> String {
    ...
    for paper in papers {
        let p = &paper.paper;
        let entry_type = if p.conference.is_some() {
            "inproceedings"
        } else {
            "article"
        };
