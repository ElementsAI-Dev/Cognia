...
fn test_collection_creation() {
    ...
    assert_eq!(collection.description, Some("AI papers collection".to_string()));
...
fn test_aggregated_search_result() {
    ...
    provider_results.insert("arxiv".to_string(), ProviderSearchResult {
        count: 5,
        success: true,
        error: None,
    });
    provider_results.insert("semantic_scholar".to_string(), ProviderSearchResult {
        count: 0,
        success: false,
    ...
    assert!(!result.provider_results.get("semantic_scholar").unwrap().success);
}
