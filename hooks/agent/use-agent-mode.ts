...
export function useAgentMode(options: UseAgentModeOptions = {}): UseAgentModeResult {
  ...
  // Get mode by ID
  const getModeById = useCallback((id: string): UnifiedAgentMode | undefined => {
  ...
  // Get modes by type
  const getModesByType = useCallback((type: 'built-in' | 'custom' | 'plugin'): UnifiedAgentMode[] => {
    switch (type) {
      case 'built-in':
  ...
  const isCustomMode = useCallback((id: string): boolean => {
    return id in customModesMap;
  }, [customModesMap]);

  const isPluginMode = useCallback((id: string): boolean => {
  ...
  const getModeSystemPrompt = useCallback((id: string): string => {
    const mode = getModeById(id);
    return mode?.systemPrompt || '';
  }, [getModeById]);

  // Get mode tools
  const getModeTools = useCallback((id: string): string[] => {
    const mode = getModeById(id);
    return mode?.tools || [];
  }, [getModeById]);

  // Get mode output format
  const getModeOutputFormat = useCallback((id: string): 'text' | 'code' | 'html' | 'react' | 'markdown' => {
    const mode = getModeById(id);
