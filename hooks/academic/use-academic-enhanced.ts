...
type SupportedAcademicProvider = 'arxiv' | 'semantic-scholar' | 'core' | 'openalex' | 'dblp' | 'huggingface-papers';
import { 
...
export interface UseAcademicEnhancedReturn {
  ...
  // Enhanced search
  searchPapersEnhanced: (query: string, options?: Partial<AcademicSearchInput>) => Promise<AcademicSearchResult>;
  ...
  createAnalysisUI: (paper: Paper, analysisType: PaperAnalysisType, content: string) => string | null;
  createComparisonUI: (papers: Paper[], content: string) => string | null;
  
  // Analysis
  analyzePaperEnhanced: (paper: Paper | LibraryPaper, analysisType: PaperAnalysisType) => Promise<AcademicAnalysisResult>;
  ...
  analyzeAndDisplay: (paper: Paper | LibraryPaper, analysisType: PaperAnalysisType) => Promise<string | null>;
...
export function useAcademicEnhanced(
   ...
   */
  const searchPapersEnhanced = useCallback(async (
    query: string,
  ...
  const createSearchResultsUI = useCallback((
    ...
    const { surfaceId, messages } = createSearchResultsSurface(
      papers,
      query,
      papers.length
    );
   ...
   */
  const createPaperCardUI = useCallback((paper: Paper): string | null => {
    if (!enableA2UI) return null;
  ...
  const createComparisonUI = useCallback((
    ...
    const { surfaceId, messages } = createPaperComparisonSurface(
      paperData,
      content
    );
    a2ui.processMessages(messages);
   ...
   */
  const analyzePaperEnhanced = useCallback(async (
   ...
   */
  const searchWebForPaper = useCallback(async (paper: Paper): Promise<void> => {
   ...
   */
  const findRelatedPapers = useCallback(async (paper: Paper): Promise<Paper[]> => {
    const keywords = [
      ...(paper.keywords || []),
      ...(paper.fieldsOfStudy || []),
    ].slice(0, 3);
    ...
    const searchQuery = keywords.length > 0
      ? keywords.join(' ')
      : paper.title.split(' ').slice(0, 5).join(' ');
   ...
   */
  const searchAndDisplay = useCallback(async (query: string): Promise<string | null> => {
   ...
   */
  const analyzeAndDisplay = useCallback(async (
    paper: Paper | LibraryPaper,
    analysisType: PaperAnalysisType
  ): Promise<string | null> => {
