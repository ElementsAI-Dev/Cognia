...
describe('useKnowledgeMap', () => {
  ...
  describe('Content Generation', () => {
    ...
    it('should call convertPDFToKnowledgeMap', async () => {
      ...
      await act(async () => {
        await result.current.convertPDFToKnowledgeMap('/path/to/file.pdf', {
          extractFigures: true,
        });
      });

      expect(mockStore.convertPDFToKnowledgeMap).toHaveBeenCalledWith('/path/to/file.pdf', {
        extractFigures: true,
      });
    });
  });
  ...
  describe('Trace Management', () => {
    ...
    it('should call updateTrace with active knowledge map id', () => {
      ...
      expect(mockStore.updateTrace).toHaveBeenCalledWith('active-km', 'trace-1', {
        title: 'Updated Trace',
      });
    });
