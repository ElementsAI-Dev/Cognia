...
export function useVideoSubtitles(options: UseVideoSubtitlesOptions = {}): UseVideoSubtitlesReturn {
  ...
  const currentCue = useMemo(() => {
    ...
    return (
      activeTrack.cues.find((cue) => cue.startTime <= currentTime && cue.endTime >= currentTime) ||
      null
    );
  ...
  const addTrack = useCallback(
    (track: Omit<SubtitleTrackState, 'id'>): string => {
      const id = generateId();
  ...
  const updateCue = useCallback(
    (trackId: string, cueId: string, updates: Partial<SubtitleCue>) => {
      setTracks((prev) =>
        prev.map((track) => {
          if (track.id !== trackId) return track;

          const cues = track.cues.map((c) => (c.id === cueId ? { ...c, ...updates } : c));
  ...
  const splitCue = useCallback(
    (trackId: string, cueId: string, atTime: number): string[] => {
      const track = tracks.find((t) => t.id === trackId);
  ...
  const mergeCues = useCallback(
    (trackId: string, cueIds: string[]): string => {
  ...
  const getCueAtTime = useCallback(
    (trackId: string, timeMs: number): SubtitleCue | null => {
      const track = tracks.find((t) => t.id === trackId);
      if (!track) return null;

      return track.cues.find((cue) => cue.startTime <= timeMs && cue.endTime >= timeMs) || null;
    },
    [tracks]
  );

  const getCuesInRange = useCallback(
    (trackId: string, startMs: number, endMs: number): SubtitleCue[] => {
      const track = tracks.find((t) => t.id === trackId);
      if (!track) return [];

      return track.cues.filter((cue) => cue.startTime < endMs && cue.endTime > startMs);
    },
    [tracks]
  );
  ...
  const syncToPlayhead = useCallback(
    (trackId: string, cueId: string, timeMs: number) => {
      const track = tracks.find((t) => t.id === trackId);
      const cue = track?.cues.find((c) => c.id === cueId);
      if (!cue) return;
  ...
  const loadFromFile = useCallback(
    async (content: string, format?: SubtitleFormat): Promise<SubtitleTrackState | null> => {
      setIsLoading(true);
      setError(null);
  ...
  const exportTrack = useCallback(
    (trackId: string, format: SubtitleFormat): string => {
      const track = tracks.find((t) => t.id === trackId);
      if (!track) return '';
  ...
  const exportAllTracks = useCallback(
    (format: SubtitleFormat): Record<string, string> => {
  ...
  const searchCues = useCallback(
    (query: string, trackId?: string): SubtitleCue[] => {
      const searchTracks = trackId ? tracks.filter((t) => t.id === trackId) : tracks;
  ...
  const validateTrack = useCallback(
    (trackId: string): { isValid: boolean; errors: string[] } => {
      const track = tracks.find((t) => t.id === trackId);
      if (!track) {
