...
import type { MentionItem, SelectedMention, MentionState, ParsedToolCall } from '@/types/mcp';
import {
  createToolMention,
...
export function useMention(options: UseMentionOptions = {}): UseMentionReturn {
  ...
  const selectMention = useCallback(
    (item: MentionItem): { newText: string; newCursorPosition: number } => {
      const displayText = formatMentionDisplay(item);
      const beforeTrigger = currentText.slice(0, mentionState.triggerPosition);
      const afterQuery = currentText.slice(
        mentionState.triggerPosition + mentionState.query.length + 1
      );
  ...
  const getSelectedMentions = useCallback((): SelectedMention[] => {
    ...
    while ((match = mentionRegex.exec(currentText)) !== null) {
      ...
      if (!name) {
        // Server mention
        mentionItem = allMentions.find((m) => m.type === 'server' && m.serverId === serverId);
