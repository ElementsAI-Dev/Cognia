/**
 * useJupyterNotebook - State management hook for Jupyter notebook rendering
 * Extracted from jupyter-renderer.tsx to separate state logic from UI
 */
...
interface UseJupyterNotebookOptions {
  content: string;
  onNotebookChange?: (content: string) => void;
}

export function useJupyterNotebook({ content, onNotebookChange }: UseJupyterNotebookOptions) {
  ...
  const notebook = useMemo<JupyterNotebook | null>(() => {
    try {
  ...
  const toggleCellCollapse = useCallback((index: number) => {
