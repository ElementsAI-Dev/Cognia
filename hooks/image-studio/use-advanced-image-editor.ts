/**
 * useAdvancedImageEditor - Extended image editor with worker and WebGL acceleration
 *
...
export interface AdvancedEditorOptions extends UseImageEditorOptions {
  useWorker?: boolean;
  useWebGL?: boolean;
  workerCount?: number;
...
export interface UseAdvancedImageEditorReturn extends UseImageEditorReturn {
  // Performance
...
export function useAdvancedImageEditor(
  ...
  const applyLevelsAdjustment = useCallback(
    async (levelOptions: LevelsOptions): Promise<void> => {
      ...
      const result = await processWithBestMethod(
        ...
        () =>
          glProcessorRef.current!.processLevels(imageData, {
            inputBlack: levelOptions.inputBlack,
            inputWhite: levelOptions.inputWhite,
            inputGamma: levelOptions.inputGamma,
            outputBlack: levelOptions.outputBlack,
            outputWhite: levelOptions.outputWhite,
          }),
  ...
  const applyCurvesAdjustment = useCallback(
    async (curvesOptions: CurvesOptions): Promise<void> => {
      ...
      const result = await processWithBestMethod(
        ...
        () => {
          // Create RGBA LUT texture from curve options
          const rgbLut = createCurveLUT(curvesOptions.rgb);
          const redLut = curvesOptions.red ? createCurveLUT(curvesOptions.red) : rgbLut;
          const greenLut = curvesOptions.green ? createCurveLUT(curvesOptions.green) : rgbLut;
          const blueLut = curvesOptions.blue ? createCurveLUT(curvesOptions.blue) : rgbLut;

          // Combine into RGBA texture (256x1)
          const rgbaLut = new Uint8Array(256 * 4);
          for (let i = 0; i < 256; i++) {
            rgbaLut[i * 4] = redLut[rgbLut[i]];
            rgbaLut[i * 4 + 1] = greenLut[rgbLut[i]];
            rgbaLut[i * 4 + 2] = blueLut[rgbLut[i]];
            rgbaLut[i * 4 + 3] = 255;
          }

          return glProcessorRef.current!.processCurves(imageData, rgbaLut);
        },
  ...
  const applyNoiseReductionAdjustment = useCallback(
    async (noiseOptions: NoiseReductionOptions): Promise<void> => {
      ...
      const result = await processWithBestMethod(
        ...
        () =>
          glProcessorRef.current!.processNoiseReduction(
            imageData,
            noiseOptions.strength ?? 50,
            noiseOptions.preserveDetail ?? 0.1
          ),
...
export default useAdvancedImageEditor;
