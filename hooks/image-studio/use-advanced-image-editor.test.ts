/**
 * Tests for useAdvancedImageEditor hook
 */
...
// Import after mocks
import { useAdvancedImageEditor } from './use-advanced-image-editor';

describe('useAdvancedImageEditor', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('initialization', () => {
    ...
    it('should accept useWorker option', () => {
      const { result } = renderHook(() =>
        useAdvancedImageEditor({ useWorker: false })
      );
    ...
    it('should accept useWebGL option', () => {
      const { result } = renderHook(() =>
        useAdvancedImageEditor({ useWebGL: false })
      );
    ...
    it('should accept workerCount option', () => {
      const { result } = renderHook(() =>
        useAdvancedImageEditor({ workerCount: 4 })
      );
      expect(result.current.isWorkerReady).toBe(true);
    });
  });
  ...
  describe('performance tracking', () => {
    it('should call onPerformanceMetric callback', async () => {
      ...
      const { result } = renderHook(() =>
        useAdvancedImageEditor({ onPerformanceMetric })
      );
  ...
  describe('cleanup', () => {
    it('should cleanup on unmount', () => {
      const { unmount } = renderHook(() => useAdvancedImageEditor());
