...
function looksLikeChartData(content: string): boolean {
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      ...
      return typeof first === 'object' && ('name' in first || 'label' in first) && ('value' in first || 'count' in first);
    }
...
function generateTitle(content: string, language?: string, type?: ArtifactType): string {
  ...
  const functionMatch = content.match(/(?:export\s+)?(?:default\s+)?(?:function|const|class)\s+(\w+)/);
  if (functionMatch) {
  ...
  if (type === 'mermaid') {
    const mermaidMatch = content.match(/^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|erDiagram|gantt|pie|mindmap)/m);
    if (mermaidMatch) {
...
export function useArtifactDetection(
  ...
  const artifacts = useMemo(() => {
    ...
    while ((match = codeBlockRegex.exec(content)) !== null) {
      ...
      if ((langLower === 'javascript' || langLower === 'typescript' || langLower === 'js' || langLower === 'ts') 
