...
description: Write Convex queries, mutations, actions, and HTTP actions. Use when creating backend functions for data fetching (queries), data modification (mutations), external API calls (actions), or HTTP endpoints. Covers argument/return validation with `v` validators, internal vs public functions, scheduling functions with `ctx.scheduler`, transactional semantics, and the `"use node"` directive for Node.js APIs.
---
