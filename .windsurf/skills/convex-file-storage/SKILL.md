...
description: Handle file uploads, storage, serving, and management in Convex. Use when implementing file upload flows (generateUploadUrl → POST → saveFile), serving files via URL (ctx.storage.getUrl), storing AI-generated or server-side files from actions (ctx.storage.store), deleting files, accessing file metadata from _storage system table, building image uploaders with preview, or managing user avatars and post attachments.
---
