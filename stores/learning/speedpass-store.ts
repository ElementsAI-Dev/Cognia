...
interface SpeedPassState {
  ...
  addCourseTextbookMapping: (mapping: Omit<CourseTextbookMapping, 'id' | 'createdAt' | 'updatedAt'>) => void;
  getRecommendedTextbooks: (courseId: string, universityId?: string) => Textbook[];
  
  // Parse progress
  setParseProgress: (progress: { textbookId: string; status: TextbookParseStatus; progress: number; message?: string } | null) => void;
  ...
  persist(
    (set, get) => ({
      ...
      removeEnrolledCourse: (courseId) => {
        set((state) => {
          ...
          return {
            academicProfile: {
              ...state.academicProfile,
              enrolledCourses: state.academicProfile.enrolledCourses.filter((id) => id !== courseId),
            },
          };
        });
      },
      ...
      getRecommendedTextbooks: (courseId, universityId) => {
        ...
        return mappings
          .map((m) => state.textbooks[m.textbookId])
            ...
            .map((kp, index) => {
              ...
              return {
                ...
                examples: relatedExamples.slice(0, 3).map((ex) => ({
                  ...
                  difficulty: ex.difficulty < 0.3 ? 'easy' : ex.difficulty < 0.7 ? 'medium' : 'hard',
                ...
                commonMistakes: [],
                memoryTips: [],
                estimatedMinutes: modeConfig.tutorialDepth === 'brief' ? 5 : modeConfig.tutorialDepth === 'standard' ? 10 : 15,
              } as TutorialSection;
            });
      ...
      updateTutorialProgress: (tutorialId, sectionId) => {
        set((state) => {
          ...
          const progress = Math.round((completedSectionIds.length / tutorial.sections.length) * 100);
      ...
      deleteTutorial: (tutorialId) => {
        set((state) => {
          ...
          return {
            tutorials: rest,
            currentTutorialId: state.currentTutorialId === tutorialId ? null : state.currentTutorialId,
          };
        });
      },
      ...
      endStudySession: (sessionId) => {
        set((state) => {
          ...
          const now = new Date();
          const totalTime = now.getTime() - new Date(session.startedAt).getTime() - session.totalPausedMs;
      ...
      createQuiz: (input) => {
        ...
        const quiz: Quiz = {
          ...
          questions: selectedQuestions.map((q) => ({
            id: nanoid(),
            sourceQuestion: q,
            source: q.sourceType === 'example' ? 'textbook_example' : 
                    q.sourceType === 'exercise' ? 'textbook_exercise' :
                    q.sourceType === 'exam' ? 'past_exam' : 'ai_generated',
            hintsUsed: 0,
            hintsAvailable: 3,
          })),
      ...
      answerQuestion: (quizId, questionIndex, answer) => {
        set((state) => {
          ...
          if (sourceQuestion.solution) {
            isCorrect = answer.trim().toLowerCase() === sourceQuestion.solution.answer.trim().toLowerCase();
          }
        ...
        set((state) => {
          ...
          return {
            ...
            globalStats: {
              ...
              totalQuestionsAttempted: state.globalStats.totalQuestionsAttempted + attemptedQuestions.length,
              totalQuestionsCorrect: state.globalStats.totalQuestionsCorrect + correctQuestions.length,
              averageAccuracy: Math.round(
                ((state.globalStats.totalQuestionsCorrect + correctQuestions.length) /
                  (state.globalStats.totalQuestionsAttempted + attemptedQuestions.length)) * 100
              ) || 0,
            },
          };
        });
