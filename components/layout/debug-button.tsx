...
import { Bug, Terminal, Activity, RotateCcw, Cpu, HardDrive, Zap, Network, Eye, Layers } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  const logStoreState = useCallback(async () => {
    ...
    try {
      ...
      const stores = await Promise.all([
        import('@/stores').then(m => ({ name: 'Settings', state: m.useSettingsStore.getState() })),
        import('@/stores').then(m => ({ name: 'Session', state: m.useSessionStore.getState() })),
        import('@/stores/chat').then(m => ({ name: 'ChatWidget', state: m.useChatWidgetStore.getState() })),
        import('@/stores/context').then(m => ({ name: 'Selection', state: m.useSelectionStore.getState() })),
      ]);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-64">
          <DropdownMenuLabel className="flex items-center gap-2 text-xs text-amber-500">
            <Bug className="h-3.5 w-3.5" />
            Debug Tools
            <Badge variant="outline" className="ml-auto text-[10px] h-4 px-1">DEV</Badge>
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          
          {/* DevTools */}
          <DropdownMenuItem onClick={openDevTools} disabled={!isTauri}>
            <Terminal className="mr-2 h-4 w-4" />
            Open DevTools
            {!isTauri && <span className="ml-auto text-[10px] text-muted-foreground">Tauri only</span>}
          </DropdownMenuItem>
          ...
          <DropdownMenuItem onClick={togglePerfOverlay}>
            ...
            {showPerfOverlay ? 'Hide' : 'Show'} Perf Overlay
          </DropdownMenuItem>
          ...
          <div className="px-2 py-1.5 text-[10px] text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              {isTauri ? 'Tauri Desktop' : 'Browser'} | {isDevMode ? 'Development' : 'Production'}
            </div>
          </div>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      {showPerfOverlay && (
        <div className="fixed bottom-4 right-4 z-[9999] bg-black/90 text-white text-xs font-mono p-3 rounded-lg shadow-xl border border-white/10">
          <div className="flex items-center gap-2 mb-2 text-amber-400">
            ...
            Performance Monitor
            <button
              onClick={() => setShowPerfOverlay(false)}
