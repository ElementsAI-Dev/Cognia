...
describe('DesignerBrowser', () => {
  ...
  it('should update results count when filtering', async () => {
    ...
    expect(screen.getByText('1 templatesFound')).toBeInTheDocument();
  });
  ...
  it('should show empty state when no templates match filters', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('searchTemplates');
    await userEvent.type(searchInput, 'zzzznonexistentzzzz');
  ...
  it('should show clear filters button when filters are active', async () => {
    ...
    // clearFilters appears in both filter bar and empty state
    const clearFiltersElements = screen.getAllByText('clearFilters');
    expect(clearFiltersElements.length).toBeGreaterThan(0);
  });
