...
describe('MCPCallDetails', () => {
  ...
  it('shows tool description when provided', () => {
    render(<MCPCallDetails {...defaultProps} toolDescription="Read contents of a file" />);
  ...
  it('shows result section when result provided', () => {
    render(<MCPCallDetails {...defaultProps} result={{ content: 'file content' }} />);
    expect(screen.getByText('result')).toBeInTheDocument();
  });

  it('shows error section when error provided', () => {
    render(<MCPCallDetails {...defaultProps} error="File not found" />);
  ...
  it('shows duration when timestamps provided', () => {
    ...
    render(<MCPCallDetails {...defaultProps} startedAt={startedAt} endedAt={endedAt} />);
    expect(screen.getByText(/1\.\d+s/)).toBeInTheDocument();
  });

  it('shows metadata section when metadata provided', () => {
    render(<MCPCallDetails {...defaultProps} metadata={{ version: '1.0', timeout: 5000 }} />);
  ...
  it('expands/collapses sections on click', () => {
    render(<MCPCallDetails {...defaultProps} result={{ data: 'test' }} />);
