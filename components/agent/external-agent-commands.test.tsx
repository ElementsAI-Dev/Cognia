...
describe('ExternalAgentCommands', () => {
  ...
  it('should render nothing when commands is undefined', () => {
    const { container } = renderWithIntl(
      <ExternalAgentCommands
        commands={undefined as unknown as AcpAvailableCommand[]}
  ...
  it('should render commands button with count badge', () => {
    renderWithIntl(<ExternalAgentCommands commands={mockCommands} onExecute={mockOnExecute} />);
  ...
  it('should be disabled when isExecuting is true', () => {
    renderWithIntl(
      <ExternalAgentCommands commands={mockCommands} onExecute={mockOnExecute} isExecuting={true} />
    );
  ...
  it('should call onExecute with command when execute button is clicked', async () => {
    ...
    const executeButtons = screen
      .getAllByRole('button')
      .filter((btn) => btn.querySelector('svg.lucide-chevron-right'));
  ...
  it('should handle single command correctly', () => {
    const singleCommand: AcpAvailableCommand[] = [{ name: 'help', description: 'Show help' }];

    renderWithIntl(<ExternalAgentCommands commands={singleCommand} onExecute={mockOnExecute} />);
