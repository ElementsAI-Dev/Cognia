...
export function AcademicStats() {
  ...
  const stats = useMemo(() => {
    ...
    const avgRating =
      withRating > 0
        ? libraryPapers
            .filter((p) => p.userRating)
            .reduce((sum, p) => sum + (p.userRating || 0), 0) / withRating
        : 0;

    const withNotes = libraryPapers.filter(
      (p) => p.userNotes || (p.notes && p.notes.length > 0)
    ).length;
    const withAnnotations = libraryPapers.filter(
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Progress
              value={stats.total > 0 ? (stats.unread / stats.total) * 100 : 0}
              className="h-2"
            />
          </div>
          <div className="space-y-2">
            ...
            <Progress
              value={stats.total > 0 ? (stats.reading / stats.total) * 100 : 0}
          ...
          <div className="space-y-2">
            ...
            <Progress
              value={stats.total > 0 ? (stats.archived / stats.total) * 100 : 0}
