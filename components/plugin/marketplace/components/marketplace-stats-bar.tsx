...
export function MarketplaceStatsBar({ stats }: MarketplaceStatsBarProps) {
  const t = useTranslations('pluginMarketplace');
  const data = stats || MARKETPLACE_STATS;
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
      <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-gradient-to-br from-blue-500/10 to-blue-500/5 border border-blue-500/20 hover:border-blue-500/40 transition-colors">
        ...
        <div>
          <div className="text-base sm:text-lg font-bold">{data.totalPlugins.toLocaleString()}</div>
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t('stats.plugins')}</div>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20 hover:border-emerald-500/40 transition-colors">
        ...
        <div>
          <div className="text-base sm:text-lg font-bold">{formatDownloads(data.totalDownloads)}</div>
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t('stats.downloads')}</div>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-gradient-to-br from-purple-500/10 to-purple-500/5 border border-purple-500/20 hover:border-purple-500/40 transition-colors">
        ...
        <div>
          <div className="text-base sm:text-lg font-bold">{data.totalDevelopers}</div>
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t('stats.developers')}</div>
        </div>
      </div>
      <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-gradient-to-br from-amber-500/10 to-amber-500/5 border border-amber-500/20 hover:border-amber-500/40 transition-colors">
        ...
        <div>
          <div className="text-base sm:text-lg font-bold">+{data.weeklyNewPlugins}</div>
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t('stats.thisWeek')}</div>
        </div>
      </div>
    </div>
  );
