...
describe('EnhancedTable', () => {
  ...
  it('renders without crashing', () => {
    render(<EnhancedTable {...defaultProps} />);
  ...
  it('displays title when provided', () => {
    render(<EnhancedTable {...defaultProps} title="Users Table" />);
    expect(screen.getByText('Users Table')).toBeInTheDocument();
  });

  it('displays search input when searchable', () => {
    render(<EnhancedTable {...defaultProps} searchable={true} />);
    expect(screen.getByPlaceholderText('Search...')).toBeInTheDocument();
  });

  it('hides search input when searchable is false', () => {
    render(<EnhancedTable {...defaultProps} searchable={false} />);
  ...
  it('sorts columns when clicked', () => {
    render(<EnhancedTable {...defaultProps} sortable={true} />);
  ...
  it('displays export dropdown when exportable', () => {
    render(<EnhancedTable {...defaultProps} exportable={true} />);
    expect(screen.getByTestId('dropdown')).toBeInTheDocument();
  });

  it('hides export dropdown when exportable is false', () => {
    render(<EnhancedTable {...defaultProps} exportable={false} />);
  ...
  it('displays pagination when enabled and has multiple pages', () => {
    ...
    render(<EnhancedTable headers={defaultHeaders} rows={manyRows} pageSize={10} />);
  ...
  it('hides pagination when disabled', () => {
    const manyRows = Array.from({ length: 25 }, (_, i) => [`Person ${i}`, i, `City ${i}`]);
    render(<EnhancedTable headers={defaultHeaders} rows={manyRows} showPagination={false} />);
  ...
  it('formats boolean values correctly', () => {
    ...
    render(<EnhancedTable headers={['Name', 'Active', 'Deleted']} rows={booleanRows} />);
  ...
  it('formats large numbers with commas', () => {
    ...
    render(<EnhancedTable headers={['Name', 'Count', 'Type']} rows={numberRows} />);
    expect(screen.getByText('1,000,000')).toBeInTheDocument();
  });

  it('formats decimal numbers to 2 places', () => {
    const decimalRows = [['Test', 3.14159, 'Value']];
    render(<EnhancedTable headers={['Name', 'Pi', 'Type']} rows={decimalRows} />);
    expect(screen.getByText('3.14')).toBeInTheDocument();
  });

  it('handles null and undefined values', () => {
    const nullRows = [['Test', null, undefined]];
    render(<EnhancedTable headers={['Name', 'Null', 'Undefined']} rows={nullRows} />);
    // Should render without crashing, null/undefined shown as empty
    expect(screen.getByText('Test')).toBeInTheDocument();
  });

  it('hides toolbar when showToolbar is false', () => {
    render(<EnhancedTable {...defaultProps} showToolbar={false} />);
    expect(screen.queryByPlaceholderText('Search...')).not.toBeInTheDocument();
  ...
  it('applies custom className', () => {
    const { container } = render(
      <EnhancedTable {...defaultProps} className="custom-class" />
    );
    expect(container.querySelector('.custom-class')).toBeInTheDocument();
  });

  it('displays no data message when rows is empty', () => {
    render(<EnhancedTable headers={defaultHeaders} rows={[]} />);
