        ...
        <div
          className={cn(
            ...
            compact
              ...
              : 'h-6 w-6 sm:h-8 sm:w-8 [&>svg]:h-3.5 [&>svg]:w-3.5 sm:[&>svg]:h-4 sm:[&>svg]:w-4'
          )}
        >
          {suggestion.icon}
        </div>
        <span className={cn('font-semibold', compact ? 'text-xs' : 'text-xs sm:text-sm')}>
          {suggestion.title}
        </span>
      </div>
      <p
...
export function WelcomeState({
  ...
  const {
    ...
    maxSuggestionsPerMode,
    useCustomQuickAccess,
    quickAccessLinks,
    customGreeting,
    customDescription,
  } = welcomeSettings;
  ...
  // Build suggestions: merge custom with defaults based on settings
  const suggestions: SuggestionCard[] = useMemo(() => {
    // Get default suggestions
    const defaultSuggestions = suggestionKeyMap[mode].map(({ icon, key }) => ({
      icon,
    ...
    // Get custom suggestions for this mode
    const modeCustomSuggestions = customSuggestions[mode]
      .filter((s) => s.enabled)
      .map((s) => ({
        icon: <Sparkles className="h-5 w-5" />, // Simple icon for custom
        title: s.title,
        description: s.description,
        prompt: s.prompt,
      }));

    // Merge based on hide defaults setting
    if (hideDefaultSuggestions && modeCustomSuggestions.length > 0) {
      return modeCustomSuggestions.slice(0, maxSuggestionsPerMode);
    }

    // Custom first, then defaults
    return [...modeCustomSuggestions, ...defaultSuggestions].slice(0, maxSuggestionsPerMode);
  }, [mode, t, customSuggestions, hideDefaultSuggestions, maxSuggestionsPerMode]);

  // Get quick access links
  const activeQuickAccessLinks = useMemo(() => {
    if (useCustomQuickAccess && quickAccessLinks.length > 0) {
      return quickAccessLinks.filter((l) => l.enabled);
    }
      ...
      <div className="w-full max-w-3xl space-y-3 sm:space-y-4">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        className="gap-1.5 rounded-full hover:bg-accent transition-colors h-7 sm:h-8 text-xs px-2.5 sm:px-3"
                      >
        ...
        {mode === 'agent' && onAgentModeChange && (
          <div className="flex justify-center">
            <AgentModeSelector selectedModeId={agentModeId} onModeChange={onAgentModeChange} />
          </div>
        )}
        ...
        {effectiveSectionsVisibility.quickAccess && activeQuickAccessLinks.length > 0 && (
          <div
            className={cn(
              'grid gap-2',
              activeQuickAccessLinks.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
            )}
          >
            {activeQuickAccessLinks.map((link) => (
              <Button key={link.id} asChild variant="ghost" className="h-auto p-0">
                ...
                >
                  <div
                    className={cn(
                      'flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg transition-colors group-hover:text-white',
                      link.color === 'purple'
