...
describe('QuickReplyBar', () => {
  ...
  it('calls onSelect when suggestion is clicked', () => {
    ...
    const suggestionButtons = screen.getAllByRole('button').filter(
      btn => !btn.querySelector('svg.animate-spin') && btn.textContent !== ''
    );
  ...
  it('applies custom className', () => {
    const { container } = render(
      <QuickReplyBar {...defaultProps} className="custom-class" />
    );
  ...
  it('generates context-aware suggestions for code content', async () => {
    const codeMessages: UIMessage[] = [
      { id: 'msg-1', role: 'assistant', content: 'Here is some code:\n```javascript\nfunction test() {}\n```', createdAt: new Date() },
    ];
  ...
  it('generates context-aware suggestions for questions', async () => {
    const questionMessages: UIMessage[] = [
      { id: 'msg-1', role: 'assistant', content: 'Would you like me to continue?', createdAt: new Date() },
    ];
  ...
  it('has a refresh button', () => {
    ...
    const buttons = screen.getAllByRole('button');
    const refreshButton = buttons.find(btn => btn.className?.includes('h-5'));
