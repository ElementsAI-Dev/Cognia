  ...
  test('should convert units', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      const convertLength = (value: number, from: string, to: string): number => {
        const fromFactor = conversions.length[from] as number;
        const toFactor = conversions.length[to] as number;
        return (value * fromFactor) / toFactor;
      };

      const convertWeight = (value: number, from: string, to: string): number => {
        const fromFactor = conversions.weight[from] as number;
        const toFactor = conversions.weight[to] as number;
        return (value * fromFactor) / toFactor;
      };
