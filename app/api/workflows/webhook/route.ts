...
function resolveWorkflowId(request: NextRequest, body: WebhookBody): string | undefined {
  const queryWorkflowId = request.nextUrl.searchParams.get('workflowId')?.trim();
  if (queryWorkflowId) {
    return queryWorkflowId;
  }

  const bodyWorkflowId = typeof body.workflowId === 'string' ? body.workflowId.trim() : '';
  if (bodyWorkflowId) {
    return bodyWorkflowId;
  }

  const headerWorkflowId = request.headers.get('x-workflow-id')?.trim();
  if (headerWorkflowId) {
...
export async function GET(request: NextRequest) {
  const workflowId =
    request.nextUrl.searchParams.get('workflowId') || request.headers.get('x-workflow-id');
...
export async function POST(request: NextRequest) {
  ...
  const workflowId = resolveWorkflowId(request, body);
  if (!workflowId) {
    return NextResponse.json(
      {
        error: 'Missing workflowId. Supply `workflowId` in query, body, or `x-workflow-id` header.',
      },
      { status: 400 }
    );
  }
