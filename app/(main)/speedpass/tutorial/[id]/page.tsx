...
export default function TutorialDetailPage() {
  ...
  const currentSection = useMemo(() => {
    if (!tutorial?.sections) return null;
    return tutorial.sections[currentSectionIndex] || null;
  }, [tutorial, currentSectionIndex]);

  // Calculate progress
  const progress = useMemo(() => {
    if (!tutorial?.sections?.length) return 0;
    return Math.round((allCompletedSections.size / tutorial.sections.length) * 100);
  }, [tutorial, allCompletedSections]);
  ...
  }, [currentSection, currentSectionIndex, tutorial, completedSections.size]);
          ...
          <ScrollArea className="h-full">
            <div className="p-4">
              ...
              <div className="space-y-1">
                {(tutorial.sections as TutorialSection[])?.map((section, sectionIndex) => (
                  <button
                    key={section.id}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">
              {currentSectionIndex + 1} / {tutorial.sections?.length || 0}
            </span>
          </div>
          {currentSectionIndex === (tutorial.sections?.length || 0) - 1 ? (
