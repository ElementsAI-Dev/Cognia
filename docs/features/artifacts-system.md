...
- Benchmark source of truth: `docs/reference/artifacts-benchmark-scorecard.md`
- Current focus patterns:
  - `AP-01` Dedicated artifact workspace continuity
  - `AP-02` Capability-declared runtime safety
  - `AP-03` Deterministic creation lineage
- This guide reflects the current implementation baseline for:
  - centralized artifact workspace state and recent-return context
  - capability-specific preview adapters and sandbox policies
  - deterministic source metadata for automatic, manual, tool, and plugin-driven artifact creation
  - explicit unsupported-action feedback for export flows
...
AI models automatically create artifacts when they generate code, documents, or other standalone content. Automatic creation now uses a shared detection pipeline so fenced code blocks, Mermaid content, and standalone math blocks flow through the same normalization path.

Automatic artifacts persist deterministic lineage metadata:

- `sourceOrigin: "auto"`
- `sourceFingerprint`: stable fingerprint derived from session, message, type, language, and normalized content
- `sourceRange`: original content offsets when available
- `userInitiated: false`

If the same message content is processed again without meaningful changes, duplicate auto-created artifacts are suppressed instead of being created again.
...
Manual creation and artifact tool/plugin creation now share the same deterministic metadata contract:

- Manual UI creation uses `sourceOrigin: "manual"` and `userInitiated: true`
- AI/tool-driven creation uses `sourceOrigin: "tool"`
- Plugin-created artifacts preserve explicit `sourceOrigin` / `userInitiated` overrides while still receiving a stable `sourceFingerprint`
...
The Artifacts list is now driven by a centralized workspace state model:

- session-scoped browsing follows the currently active chat session
- recent artifact targets are tracked so users can return to the latest artifact context
- search, type filters, and runtime filters are stored centrally instead of being local-only UI state
- opening an artifact, switching to Canvas, and reopening the Artifacts panel preserves the prior artifact return context
...
```typescript
{
  ...
  metadata?: {
    ...
    runtimeHealth?: 'ready' | 'loading' | 'error' | 'unsupported';
    exportFormats?: ('raw' | 'html' | 'svg' | 'png' | 'pdf')[];
    userInitiated?: boolean;
  }
}
```

Additional workspace state is persisted separately from individual artifacts and tracks the current scope, filters, recent artifact ids, and return context for Artifact/Canvas handoff.
...
When opening an artifact in Canvas, Cognia now records a return context snapshot so reopening the Artifacts panel restores the same artifact/workspace target instead of dropping the user back into a blank default state.
...
- `raw` export is supported through the artifact tool contract
- unsupported export formats now fail explicitly instead of reporting a false success state
...
Preview surfaces now expose a shared runtime health indicator:

- `ready`: preview rendered successfully
- `loading`: runtime container or renderer is still preparing
- `error`: preview runtime reported a recoverable error

Executable preview types use capability-specific runtime adapters and sandbox policies:

- HTML/SVG previews default to `allow-same-origin`
- React previews keep the stricter script-capable iframe path required for the component shell
- iframe/runtime failures surface a retryable error UI instead of silently leaving the preview blank
