...
export function extractLearningPatterns(
  ...
  // Analyze template structure for domain hints
  if (templateContent.includes('code') || templateContent.includes('function')) {
    domainKnowledge.push('Technical/coding context');
  }
  if (templateContent.includes('write') || templateContent.includes('create')) {
    domainKnowledge.push('Creative writing context');
  }
  if (templateContent.includes('analyze') || templateContent.includes('explain')) {
