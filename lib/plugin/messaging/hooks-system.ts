...
export class PluginEventHooks {
  ...
  dispatchExternalAgentConnect(agentId: string, agentName: string) {
    this.executeHook('onExternalAgentConnect', (hooks) =>
      (hooks as PluginHooksAll & { onExternalAgentConnect?: (agentId: string, name: string) => void })
  ...
  dispatchExternalAgentExecutionStart(agentId: string, sessionId: string, prompt: string) {
    this.executeHook('onExternalAgentExecutionStart', (hooks) =>
      (hooks as PluginHooksAll & { onExternalAgentExecutionStart?: (agentId: string, sessionId: string, prompt: string) => void })
        .onExternalAgentExecutionStart?.(agentId, sessionId, prompt)
    );
  }

  dispatchExternalAgentExecutionComplete(agentId: string, sessionId: string, success: boolean, response?: string) {
    this.executeHook('onExternalAgentExecutionComplete', (hooks) =>
      (hooks as PluginHooksAll & { onExternalAgentExecutionComplete?: (agentId: string, sessionId: string, success: boolean, response?: string) => void })
        .onExternalAgentExecutionComplete?.(agentId, sessionId, success, response)
    );
  }

  dispatchExternalAgentPermissionRequest(agentId: string, sessionId: string, toolName: string, reason?: string) {
    this.executeHook('onExternalAgentPermissionRequest', (hooks) =>
      (hooks as PluginHooksAll & { onExternalAgentPermissionRequest?: (agentId: string, sessionId: string, toolName: string, reason?: string) => void })
        .onExternalAgentPermissionRequest?.(agentId, sessionId, toolName, reason)
    );
  }

  dispatchExternalAgentToolCall(agentId: string, sessionId: string, toolName: string, args: Record<string, unknown>) {
    this.executeHook('onExternalAgentToolCall', (hooks) =>
      (hooks as PluginHooksAll & { onExternalAgentToolCall?: (agentId: string, sessionId: string, toolName: string, args: Record<string, unknown>) => void })
